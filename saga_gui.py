...
from normalized_splatting.scene import GaussianModel
from normalized_splatting.scene.cameras import Camera
from normalized_splatting.utils.graphics_utils import focal2fov, fov2focal
...
# grounding DINO
# GroundingDINO powers text-prompted segmentation. It is optional: the viewer
# runs without it, only the text prompt box is disabled.
try:
...
class CONFIG:
    ...
    store_path = f"{MODEL_PATH}/point_cloud/iteration_{SCENE_GAUSSIAN_ITERATION}/saga_{FEATURE_GAUSSIAN_ITERATION}"

    SCALE_GATE_PATH = f"{store_path}/scale_gate.pt"
    SCENE_PCD_PATH = f"{store_path}/point_cloud.ply"
...
class GaussianSplattingGUI:
    def __init__(self, opt, gaussian_model:GaussianModel, scale_gate: torch.nn.modules.container.Sequential, no_saga = False) -> None:
        self.opt = opt
        ...
        self.clickmode_3d_button = False
        ...
        # Text-prompted segmentation is opt-in: it needs the GroundingDINO package
        # and a checkpoint. Without both, the rest of the viewer still works.
        self.gd_model = None
        self.gd_transform = None
        gd_checkpoint = getattr(opt, "GROUNDING_DINO_CHECKPOINT", None) \
            or os.environ.get("GROUNDING_DINO_CHECKPOINT")
        if GROUNDING_DINO_AVAILABLE and gd_checkpoint:
            if not os.path.exists(gd_checkpoint):
                raise FileNotFoundError(
                    f"GroundingDINO checkpoint not found: {gd_checkpoint}")
            gd_config = Path(os.path.dirname(groundingdino.__file__)) / "config/GroundingDINO_SwinT_OGC.py"
            self.gd_model = gd_load_model(gd_config, gd_checkpoint)
            self.gd_transform = gd_T.Compose(
                [
            ...
            print("Text-prompted segmentation disabled "
                  "(set --grounding_dino_checkpoint or $GROUNDING_DINO_CHECKPOINT to enable).")

        
        self.no_saga = no_saga
        
        if not self.no_saga:
            engine: GaussianModel = self.engine['scene']
            engine.set_eval_mode()
    ...
    def register_dpg(self):
        ...
        def clickmode_3d_callback(sender):
            self.clickmode_3d_button = 1 - self.clickmode_3d_button
        ...
        with dpg.window(label="Control", tag="_control_window", width=300, height=550, pos=[self.window_width+10, 0]):
            ...
            dpg.add_checkbox(label="Normals", callback=render_mode_normal_callback, user_data="Some Data")


            dpg.add_text("\nSegment option: ", tag="seg")
            dpg.add_checkbox(label="clickmode", callback=clickmode_callback, user_data="Some Data")
            dpg.add_checkbox(label="clickmode_3d", callback=clickmode_3d_callback, user_data="Some Data")
            ...
            # Add a new section for text-based clustering
            dpg.add_text("Text Clustering")
            def attn_map_callback(sender):
                self.max_attn_pt_flag = not self.max_attn_pt_flag
                
            dpg.add_checkbox(label="Use Attn Map", callback=attn_map_callback, user_data="some data")
            dpg.add_input_text(label="Enter Text", tag="text_input", callback=self.update_text_input)
            dpg.add_button(label="Submit Text", callback=self.submit_text)
            dpg.add_slider_float(label="Text Threshold", tag="text_threshold_slider", min_value=0.0, max_value=1.0, default_value=self.text_threshold, callback=self.update_gd_text_threshold)
            dpg.add_slider_float(label="Box Threshold", tag="box_threshold_slider", min_value=0.0, max_value=1.0, default_value=self.box_threshold, callback=self.update_gd_box_threshold)
        ...
        def change_pos(sender, app_data):
            ...
            if (self.clickmode_button or self.clickmode_3d_button) and app_data == 1:     # in the click mode and right click
    ...
    def update_gd_text_threshold(self, sender, app_data):
        self.text_threshold = app_data

    def update_gd_box_threshold(self, sender, app_data):
        self.box_threshold = app_data

    def submit_text(self):
        if hasattr(self, "text_input") and self.text_input.strip():
            self.new_text = self.text_input
    
    def update_text_input(self, sender, app_data):
        """Update the current text input."""
        self.text_input = app_data
    ...
    def cluster_in_3D(self):
        ...
        # except:
        #     pass
        point_features = self.engine['scene'].get_saga_features
    ...
    def do_pca(self):
        sems = self.engine['scene'].get_saga_features.clone().squeeze()
        N, C = sems.shape
        torch.manual_seed(0)
        randint = torch.randint(0, N, [200_000])
        sems /= (torch.norm(sems, dim=1, keepdim=True) + 1e-6)
        sem_chosen = sems[randint, :]
        self.proj_mat = self.pca(sem_chosen, n_components=3).real
        print("project mat initialized !")


    @torch.no_grad()
    def fetch_data(self, view_camera: Camera):
        ...
        self.rendered_cluster = None if self.cluster_point_colors is None else render(view_camera, self.engine['scene'], self.opt,
                                                                                      self.bg_color, 
                                                                                      override_color=torch.from_numpy(self.cluster_point_colors).cuda().float())["render"].permute(1, 2, 0)
        # --- RGB image --- #
        ...
        # --- semantic imfage --- #
        if not self.no_saga:
            sems = scene_outputs["contrastive_features"].permute(1, 2, 0)
            H, W, C = sems.shape
            sems /= (torch.norm(sems, dim=-1, keepdim=True) + 1e-6)
            ...
            if isinstance(self.new_text, str) and len(self.new_text.strip()) > 0 \
                    and self.gd_model is not None:
                self.prompt_num=0

                # GroundingDINO takes a PIL image through its own transform stack.
                img_pil = Image.fromarray((img*255).detach().cpu().numpy().astype(np.uint8))
                gd_img, _ = self.gd_transform(img_pil, None)
                gd_img = gd_img.cuda()
                
                boxes, logits, _, max_attn_pts = gd_predict(
                    model=self.gd_model,
                    image=gd_img,
                    caption=self.new_text,
                    box_threshold=self.box_threshold,
                    text_threshold=self.text_threshold,
                    return_max_attn_points=True
                )
                ...
                if logits.numel() > 0:
                    ...
                    gd_W, gd_H = img.shape[1], img.shape[0]
                    
                    # for box in boxes:
                    box *= torch.Tensor([gd_W, gd_H, gd_W, gd_H])
                    
                    
                    xy = box[:2].tolist()
                    self.new_click_xy = xy
                    self.new_click = True
                    ...
                    features = featmap[y_min:y_max, x_min:x_max, :]
                    features_flat = features.reshape(-1, features.shape[-1]).detach().cpu().numpy()
                    img_flat = img[y_min:y_max, x_min:x_max].reshape(-1, img.shape[-1]).clamp(0,1)
                    ...
                    cluster_mask = np.zeros(img_flat.shape[0], dtype=bool)
                    cluster_mask[largest_cluster_indices] = True
                    mask_2d = cluster_mask.reshape(y_max - y_min, x_max - x_min,)
                    
                    img_with_box = (img.detach().contiguous().cpu().numpy() * 255).astype(np.uint8)
                    img_with_box = cv2.cvtColor(img_with_box, cv2.COLOR_RGB2BGR)

                    img_with_box = cv2.rectangle(img_with_box, (x_min, y_min), (x_max, y_max), color=(0, 255, 0), thickness=2)

                    img_with_box[y_min:y_max,x_min:x_max][mask_2d] = 0.5 * img_with_box[y_min:y_max,x_min:x_max][mask_2d] + 0.5*np.array([0,1,0])
                    
                    # cv2.imshow("img", img_with_box)
                    # cv2.waitKey(0)
                    # cv2.destroyAllWindows()
                    
                    # for idx, attn_map_maxes in enumerate(max_attn_pts):
                    #     max_pt_for_this_box = attn_map_maxes[max_id]
                        
                    #     offset_x = max_pt_for_this_box[0] * box[2]
                    #     offset_y = max_pt_for_this_box[1] * box[3]
                        
                    #     pt = (int(xy[0] + offset_x), int(xy[1] + offset_y))


                    #     # Draw the point on the image
                    #     img_with_point = img_with_box.copy()
                    #     img_with_point = cv2.circle(img_with_point, pt, radius=5, color=(255, 0, 0), thickness=-1)

                    #     # Save the image to the output directory
                    #     output_path = os.path.join(output_dir, f"image_with_point_{idx}.png")
                    #     cv2.imwrite(output_path, img_with_point)

                    # if self.max_attn_pt_flag:
                    #     xy = [*pt]
            ...
            if len(self.new_click_xy) > 0:
                ...
                if self.new_click:
                    ...
                    if self.clickmode_3d_button:
                        # Unproject the clicked pixel with the rendered depth, take the
                        # feature of the nearest Gaussian in 3D rather than the pixel's
                        # alpha-blended feature.
                        depth_map = scene_outputs["depth"]
                        z = depth_map[0, int(xy[1]) % H, int(xy[0]) % W].item()

                        fx, fy, cx, cy = self.camera.intrinsics
                        u, v = int(xy[0]) % W, int(xy[1]) % H
                        point_3d = np.array([(u - cx) * z / fx,
                                             (v - cy) * z / fy,
                                             z])
                        ...
                        point_world = w2c[:3, :3] @ point_3d + w2c[:3, 3]

                        new_feat = self.engine["scene"].get_nearest_feature(
                            point_world, 3).reshape(featmap.shape[-1], -1)
                    else:
                ...
                if self.segment3d_flag:
                    ...
                    """
                    self.segment3d_flag = False
                    feat_pts = self.engine['scene'].get_saga_features.squeeze()
        ...
        render_num = 0
        if self.render_mode_rgb or self.no_saga or (not self.render_mode_pca and not self.render_mode_cluster and not self.render_mode_similarity):
            self.render_buffer = rgb_score.cpu().numpy().reshape(-1)
            render_num += 1
        
        if self.render_mode_pca and not self.no_saga:
            ...
            render_num += 1
        if self.render_mode_cluster and not self.no_saga:
            ...
            render_num += 1
        if self.render_mode_similarity and not self.no_saga:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--grounding_dino_checkpoint', type=str, default=None,
                        help="Path to groundingdino_swint_ogc.pth. Enables text-prompted "
                             "segmentation. Falls back to $GROUNDING_DINO_CHECKPOINT.")
    ...
    opt.GROUNDING_DINO_CHECKPOINT = args.grounding_dino_checkpoint
    
    if args.feature_iteration == 0:
        opt.store_path = f"{opt.MODEL_PATH}/point_cloud/iteration_{opt.SCENE_GAUSSIAN_ITERATION}/"
        opt.SCALE_GATE_PATH = None
        no_saga = True
    else:
        opt.store_path = f"{opt.MODEL_PATH}/point_cloud/iteration_{opt.SCENE_GAUSSIAN_ITERATION}/saga_{opt.FEATURE_GAUSSIAN_ITERATION}"

        opt.SCALE_GATE_PATH = f"{opt.store_path}/scale_gate.pt"
        no_saga = False
        
    opt.SCENE_PCD_PATH = f"{opt.store_path}/point_cloud.ply"
    ...
    gui = GaussianSplattingGUI(opt, gs_model, scale_gate, no_saga = no_saga)
