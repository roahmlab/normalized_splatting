...
def PILtoTorch(pil_image, resolution, depth=False):
    ...
    interp = cv2.INTER_NEAREST if depth else cv2.INTER_CUBIC
    resized = cv2.resize(img, resolution, interpolation=interp)
    
    denom = 1. if depth else 255.
    resized_image = torch.from_numpy(resized).float() / denom
