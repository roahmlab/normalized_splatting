...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if cam_info.depth is not None:
        resized_depth_rgb = PILtoTorch(cam_info.depth, resolution, True)
    else:
    ...
    if cam_info.K is not None:
        # Scale intrinsics by the actual resize ratio. The old code divided by
        # args.resolution, which was wrong whenever resolution_scale != 1 and
        # silently skipped rescaling entirely at -r -1 -- where large images do
        # get downsampled to 1.6K.
        scale_x = resolution[0] / orig_w
        scale_y = resolution[1] / orig_h
        K = cam_info.K.copy()
        K[0] = cam_info.K[0] * scale_x
        K[1] = cam_info.K[1] * scale_y
    else:
        K = None
