...
from normalized_splatting.scene.gaussian_model import TrainingTarget
from normalized_splatting.utils.general_utils import safe_state
import uuid
from tqdm import tqdm
from normalized_splatting.arguments import ModelParams, PipelineParams, OptimizationParams, get_combined_args
...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint_iterations, debug_from):
    ...
    gaussians = GaussianModel(dataset.sh_degree, feature_dim=dataset.feature_dim)
    ...
    gaussians.change_to_segmentation_mode(opt, TrainingTarget.CONTRASTIVE_FEATURES, fixed_feature=False)
    ...
    gaussians.optimizer.add_param_group(param_group)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            per_pixel_weight = (per_pixel_weight - per_pixel_weight.min()) / (per_pixel_weight.max() - per_pixel_weight.min() + 1e-8) * 9. + 1.
        ...
        render_pkg_feat = render(viewpoint_cam, gaussians, pipe, background,
                                 norm_point_features=True)
        rendered_features = render_pkg_feat["contrastive_features"]
        ...
        # N_sampled_scales 32
        if scale_aware_dim <= 0 or scale_aware_dim >= 32:
            gates = scale_gate(sampled_scales.unsqueeze(-1).cuda())
        else:
            int_sampled_scales = ((1 - sampled_scales.squeeze().cuda()) * scale_aware_dim).long()
    ...
    scene.save(
        f"{scene.loaded_iter}/saga_{iteration}/",
        saga_iteration=iteration)
        
    torch.save(scale_gate.state_dict(), 
               os.path.join(scene.model_path, 
                            f"point_cloud/iteration_{scene.loaded_iter}/saga_{iteration}/scale_gate.pt"))
...
if __name__ == "__main__":
    ...
    lp = ModelParams(parser, sentinel=False)
